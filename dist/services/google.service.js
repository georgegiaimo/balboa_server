...
let GoogleService = class GoogleService {
    ...
    async getAccessToken(domain_keys, subject_email) {
    ...
    async getUsersFromDirectory(access_token, pageToken) {
        ...
        try {
            const response = await axios_1.default.get(`${baseUrl}?${params.toString()}`, {
                headers: {
    ...
    */
    /*
        public async getAccessToken2() {
            ...
            */
    async getOrganizationalUnits(access_token) {
        //var access_token = await this.getAccessToken();
        const customerId = 'my_customer';
    ...
    */
    async syncProductions() {
        ...
        for (let org_unit of google_org_units) {
            //see if can find that record in current database
            var record = productions2.find((x) => {
                //return x.name.toLowerCase().replace(/[()]/g,'').replace('/','').replace(/ /g,'') == org_unit.name.toLowerCase().replace(/[()]/g,'').replace('/','').replace(/ /g,'') 
                return x.google_org_unit_id == org_unit.orgUnitId;
            });
            if (record)
                record.exists_in_google = true;
        }
    ...
    async syncUsers() {
        ...
        google_users.forEach((user) => {
            ...
            if (org_unit)
                user.orgUnitId = org_unit.orgUnitId;
            else
                console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ can not find org unit path', user.orgUnitPath, user.name.fullName);
        });
        ...
        for (let user_gg of google_users) {
            ...
            if (record) {
                ...
                var current_asignment = assignments.find((x) => { return x.assignment_status == 'active'; });
                if (current_asignment) {
                    if (current_asignment.google_org_unit_id == user_gg.orgUnitId) {
                        ...
                        //find new production
                        var production = productions.find((x) => { return x.org_unit_path == user_gg.orgUnitPath; });
                        if (production)
                            await this.addNewProductionAssignment(production, record);
                        else
                            console.log('org unit not found in db', user_gg.orgUnitPath);
                    }
                }
                else {
                ...
                if (productionx)
                    await this.addNewProductionAssignment(productionx, { user_id: user_id });
                else
                    console.log('org unit for new user not found in db', user_gg.orgUnitPath);
            }
        }
        ...
        for (let user_gg of google_users) {
            ...
            if (!user_gg_personal_email)
                continue;
            //see if can find that record in current database
            var record = users2.find((x) => {
                return x.personal_email.toLowerCase() == user_gg_personal_email.toLowerCase();
            });
            if (record)
                record.exists_in_google = true;
        }
        var deleted_users = users2.filter((x) => { return !x.exists_in_google; });
    ...
    async getUsers(access_token) {
        ...
        do {
            // 1. Fetch the current page
            const response = await this.getUsersFromDirectory(access_token, page_token);
            // 2. Add these users to our master list
            if (response.users) {
                allUsers = allUsers.concat(response.users);
            }
...
exports.GoogleService = GoogleService = __decorate([
    ...
    __param(0, (0, tsyringe_1.inject)(google_repository_1.GoogleRepository)),
